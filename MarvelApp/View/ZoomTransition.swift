...
extension ZoomTransition: UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        let maybeBackgroundImageView = (backgroundVC as? ZoomTransition)?.zoomCharacterImageView(for: self)
        let maybeForegroundImageView = (foregroundVC as? ZoomTransition)?.zoomCharacterImageView(for: self)
...
extension ZoomTransition: UINavigationControllerDelegate {
    func navigationController(_ navigationController: UINavigationController, animationControllerFor operation: UINavigationController.Operation, from fromVC: UIViewController, to toVC: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        if fromVC is CharactersVC && toVC is CharactersVC {
